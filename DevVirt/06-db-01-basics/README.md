...
При выборе одного между ACID и BASE требукется определиться какой результат мы приследуем - высоконадёжные или высокопроизводительные системы на выходе.
...
Apache Kafka в этом смысле более надёжна, для т.к. данные делятся по разделам (partition) и чтобы соблюдать порядок доставки каждый получатель сообщений читает данные ровно из одного раздела.
